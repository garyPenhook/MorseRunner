...
uses
  ...
  SysUtils, Ini, Math, RndFunc, Contest, Log, {$ifdef FPC}Globals{$else}Main{$endif};
...
begin
  if RunMode = rmHst then
    Result := Ini.Wpm
  // In a pile-up, let each newly-created caller receive an independent
  // whole-number speed from the configured inclusive range.
  else if (Ini.Activity > 1) and
          (Ini.MultiCallerMaxWpm >= Ini.MultiCallerMinWpm) then
    Result := Ini.MultiCallerMinWpm +
      Random(Ini.MultiCallerMaxWpm - Ini.MultiCallerMinWpm + 1)
...
begin
  ...
  begin
    case State of
      osNeedPrevEnd: if {$ifdef FPC}Globals.GEdit1Text{$else}Mainform.Edit1.Text{$endif} = '' then SetState(osNeedQso);
      osNeedQso: ;
