...
uses
  ...
  Log,      // for TQso
  SSExchParser,
  Contest, Station, DxStn{$ifdef FPC}, GetDataPath{$endif};
...
type
  ...
  public
    ...
    class function compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TSweepstakesCallRec) : integer; static;
  end;
...
public
  ...
  function GetCheckSection(const ACallsign: string; AThreshold: Single = 0): String; {$ifdef FPC}override;{$endif}
...
{$ifdef FPC}var CorrectedCallsign: string;{$endif}
begin
  {$ifdef FPC}CorrectedCallsign{$else}var CorrectedCallsign: string{$endif} := ExchValidator.Call;
...
begin
  ...
  {$ifdef FPC}R{$else}var R: Single{$endif} := Random;
  if R < 0.43 then
...
class function TSweepstakesCallRec.compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TSweepstakesCallRec) : integer;
begin
