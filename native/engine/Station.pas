...
{$ifdef FPC}
var
  ExchTypesUndef : TExchTypes;  // initialized in initialization section below
{$else}
const
...
uses
  {$ifdef FPC}Globals{$else}Main{$endif},  // for BDebugCwDecoder, sbar
...
begin
  ...
  while (P > 0) do
    begin
      {$ifdef FPC}Q{$else}var Q: Integer{$endif} := P;
      ...
      if ReplaceTokenAt(AMsg, P, '<HisName>', {$ifdef FPC}Globals.GEdit2Text{$else}MainForm.Edit2.Text{$endif}) then Break;
...
var
  ...
  digits : integer;
  IsDxStation: boolean;
  {$ifdef FPC}pRange: PSerialNRSettings;{$endif}
begin
  ...
  case SimContest of
    ...
    scArrlSS:
      ...
      if Call = MyCall then
        ...
        {$ifdef FPC}pRange{$else}var pRange : PSerialNRSettings{$endif} := @Ini.SerialNRSettings[Ini.SerialNR];
...
{$ifdef FPC}
initialization
  FillByte(ExchTypesUndef, SizeOf(TExchTypes), $FF);
{$endif}
