...
uses
  ...
  Station, DxStn, Log{$ifdef FPC}, GetDataPath{$endif};
...
type
    ...
    public
        ...
        class function compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TCWSSTRec) : integer; static;
    end;
...
begin
    ...
    try
        ...
        slst.LoadFromFile({$ifdef FPC}GetDataPath.GetDataPath{$else}ParamStr(1){$endif} + 'K1USNSST.txt');
        slst.Sort;
...
class function TCWSSTRec.compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TCWSSTRec) : integer;
begin
