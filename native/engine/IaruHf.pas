...
type
  ...
  public
    ...
    class function compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TIaruHfCallRec) : integer; static;
  end;
...
begin
  ...
  try
    ...
    slst.LoadFromFile({$ifdef FPC}GetDataPath.GetDataPath{$else}ParamStr(1){$endif} + 'IARU_HF.txt');
...
class function TIaruHfCallRec.compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TIaruHfCallRec) : integer;
begin
