...
type
    ...
    public
        ...
        class function compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TCWOPSRec) : integer; static;
    end;
...
begin
    ...
    try
        ...
        slst.LoadFromFile({$ifdef FPC}GetDataPath.GetDataPath{$else}ParamStr(1){$endif} + 'CWOPS.LIST');
...
class function TCWOPSRec.compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TCWOPSRec) : integer;
begin
