...
uses
  ...
  Log{$ifdef FPC}, GetDataPath{$endif};
...
type
  ...
  public
    ...
    class function compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TArrlDxCallRec) : integer; static;
  end;
...
begin
  ...
  try
    ...
    slst.LoadFromFile({$ifdef FPC}GetDataPath.GetDataPath{$else}ParamStr(1){$endif} + 'ARRLDXCW_USDX.txt');
...
class function TArrlDxCallRec.compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TArrlDxCallRec) : integer;
begin
