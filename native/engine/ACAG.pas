...
type
  ...
  public
    ...
    class function compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TAcagCallRec) : integer; static;
  end;
...
begin
  ...
  try
    ...
    slst.LoadFromFile({$ifdef FPC}GetDataPath.GetDataPath{$else}ParamStr(1){$endif} + 'JARL_ACAG.TXT');
...
class function TAcagCallRec.compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TAcagCallRec) : integer;
begin
