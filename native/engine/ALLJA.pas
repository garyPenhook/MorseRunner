...
uses
  ...
  Generics.Defaults, Generics.Collections, Contest, DxStn, Log{$ifdef FPC}, GetDataPath{$endif};
...
type
  ...
  public
    ...
    class function compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TAllJaCallRec) : integer; static;
  end;
...
begin
  ...
  try
    ...
    slst.LoadFromFile({$ifdef FPC}GetDataPath.GetDataPath{$else}ParamStr(1){$endif} + 'JARL_ALLJA.TXT');
...
class function TAllJaCallRec.compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TAllJaCallRec) : integer;
begin
