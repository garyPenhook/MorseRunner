...
uses
  SysUtils, Generics.Defaults, {$ifdef FPC}Math{$else}System.Math{$endif},
  {$ifdef FPC}Globals{$else}Main{$endif},  // for SetMySerialNR
  Classes, DXCC;
...
begin
  ...
  if Result then
    {$ifdef FPC}if Assigned(Globals.GSetMySerialNRProc) then Globals.GSetMySerialNRProc(){$else}MainForm.SetMySerialNR{$endif};
end;
