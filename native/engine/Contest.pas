...
uses
  SndTypes, Station, StnColl, MyStn, Ini, Log, {$ifdef FPC}Classes{$else}System.Classes{$endif},
  ExchFields,
...
type
  ...
  public
    ...
    {$ifdef FPC}
    function IsCallLocalToContest(const ACallsign: string): boolean; virtual;
    function GetCheckSection(const ACallsign: string; AThreshold: Single = 0): string; virtual;
    {$endif}
...
uses
  ...
  PerlRegEx,
  {$ifdef FPC}Globals{$else}VCL.Graphics, Main{$endif}, CallLst, DXCC;

{$ifdef FPC}
const
  clDefault = Integer($20000000);  // from VCL.Graphics (Phase 2 stub)
{$endif}
...
begin
  Log.CallSent := ({$ifdef FPC}Globals.GEdit1Text{$else}Mainform.Edit1.Text{$endif} <> '') and
    ({$ifdef FPC}Globals.GEdit1Text{$else}Mainform.Edit1.Text{$endif} = Self.Me.HisCall);
end;
...
function TContest.ValidateEnteredExchange(const ACall, AExch1, AExch2: string;
  ...
  begin
    Result := false;
    case {$ifdef FPC}Globals.GRecvExchTypes{$else}Mainform.RecvExchTypes{$endif}.Exch1 of
  ...
  begin
    Result := false;
    case {$ifdef FPC}Globals.GRecvExchTypes{$else}Mainform.RecvExchTypes{$endif}.Exch2 of
...
begin
  if not ValidateExchField1(AExch1) then
    AExchError := format('Missing/Invalid %s',
      [Exchange1Settings[{$ifdef FPC}Globals.GRecvExchTypes{$else}Mainform.RecvExchTypes{$endif}.Exch1].C])
  else if not ValidateExchField2(AExch2) then
    AExchError := format('Missing/Invalid %s',
      [Exchange2Settings[{$ifdef FPC}Globals.GRecvExchTypes{$else}Mainform.RecvExchTypes{$endif}.Exch2].C])
  else
    AExchError := '';
...
begin
  ...
  if Qso.Exch2.IsEmpty and ({$ifdef FPC}Globals.GRecvExchTypes{$else}Mainform.RecvExchTypes{$endif}.Exch2 <> etNaQpNonNaExch2) then
    Qso.Exch2 := '?';
end;
...
begin
  ...
  if Me.State = stSending then
    ...
    Blk := Me.GetBlock;
    //self-mon. gain
    Smg := Power(10, ({$ifdef FPC}Globals.GVolumeSliderValue{$else}MainForm.VolumeSlider1.Value{$endif} - 1) * 3);

    // apply linear rolloff towards zero between -57 and -60db (i.e. Smg=0 @ -60db)
    if {$ifdef FPC}Globals.GVolumeSliderValue{$else}MainForm.VolumeSlider1.Value{$endif} < 0.05 then       // @ -57db, value = 3/60 = 0.05
      Smg := Smg * {$ifdef FPC}Globals.GVolumeSliderValue{$else}MainForm.VolumeSlider1.Value{$endif} * 60; // 60 dB steps in [-60,0dB]
  ...
  //save
  {$ifdef FPC}
  if (Globals.GAlWavFile1 <> nil) and Globals.GAlWavFile1.IsOpen then
    Globals.GAlWavFile1.WriteFrom(@Result[0], nil, Ini.BufSize);
  {$else}
...

{
  ...
  ShowRate;
  {$ifdef FPC}
  if Globals.GPanel2 <> nil then
    Globals.GPanel2.Caption := FormatDateTime('hh:nn:ss', BlocksToSeconds(BlockNumber) /  86400);
  if (Ini.RunMode = rmPileUp) and (Globals.GPanel4 <> nil) then
    Globals.GPanel4.Caption := Format('Pile-Up:  %d', [DxCount]);
  {$else}
...
{$ifdef DEBUG}
     if {$ifdef FPC}Globals.BDebugExchSettings{$else}Main.BDebugExchSettings{$endif} then
     begin
         {$ifdef FPC}
         Globals.GEdit1Text := DxStn.LastDxCallsign;
         Globals.GEdit2Text := '';
         Globals.GEdit3Text := '';
         {$else}
...
{$endif}
  ...
  if (BlocksToSeconds(BlockNumber) >= (Duration * 60)) or FStopPressed then
    ...
    if RunMode = rmHst then
      begin
      {$ifdef FPC}if Assigned(Globals.GRunStopProc) then Globals.GRunStopProc(){$else}MainForm.Run(rmStop){$endif};
      FStopPressed := false;
      {$ifdef FPC}if Assigned(Globals.GPopupScoreHstProc) then Globals.GPopupScoreHstProc(){$else}MainForm.PopupScoreHst{$endif};
      end
      ...
      {$ifdef FPC}if Assigned(Globals.GRunStopProc) then Globals.GRunStopProc(){$else}MainForm.Run(rmStop){$endif};
      FStopPressed := false;
      {$ifdef FPC}if Assigned(Globals.GPopupScoreWpxProc) then Globals.GPopupScoreWpxProc(){$else}MainForm.PopupScoreWpx{$endif};
      end
    else
      begin
      {$ifdef FPC}if Assigned(Globals.GRunStopProc) then Globals.GRunStopProc(){$else}MainForm.Run(rmStop){$endif};
...
{$ifdef FPC}
function TContest.IsCallLocalToContest(const ACallsign: string): boolean;
begin
  Result := True;
end;

function TContest.GetCheckSection(const ACallsign: string; AThreshold: Single = 0): string;
begin
  Result := '';
end;
{$endif}
