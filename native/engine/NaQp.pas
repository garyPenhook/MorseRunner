...
uses
  Generics.Defaults, Generics.Collections, {$ifdef FPC}ExchFields,{$endif} DualExchContest, DxStn, Log,
  Station{$ifdef FPC}, GetDataPath{$endif};
...
type
  ...
  public
    ...
    class function compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TNaQpCallRec) : integer; static;
  end;
...
public
  ...
  function IsCallLocalToContest(const ACallsign: string) : boolean; {$ifdef FPC}override;{$endif}
...
var
  rec: TNaQpCallRec;
  index: {$ifdef FPC}int64{$else}integer{$endif};
begin
  rec := TNaQpCallRec.Create();
  rec.Call := ACall;
  recOut:= nil;
...
class function TNaQpCallRec.compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TNaQpCallRec) : integer;
begin
