...
uses
  Generics.Defaults, {$ifdef FPC}Math{$else}System.Math{$endif};
...
var
  I : integer;
  {$ifdef FPC}pItem: PSerNRItem;{$endif}
begin
  Sum := 0;
  ...
  {$ifdef FPC}pItem{$else}var pItem : PSerNRItem{$endif} := @SerialNRTbl[0];
...
begin
  ...
  {$ifdef FPC}
  // Binary search by CummulativeCnt (FPC-compatible)
  Lo := 0;
  Hi := High(SerialNRTbl);
  FoundIdx := Hi;  // default: last bin
  while Lo <= Hi do begin
    Mid := (Lo + Hi) div 2;
    if SerialNRTbl[Mid].CummulativeCnt < count then
      Lo := Mid + 1
    else begin
      FoundIdx := Mid;
      Hi := Mid - 1;
