...
uses
  SysUtils, Classes, RndFunc, {$ifndef FPC}Dialogs,{$endif}
  {$ifdef FPC}Globals{$else}Main{$endif},  // for Mainform / Globals
      ...
      if State = stListening then
        ...
        if Oper.State = osFailed then begin
          ...
          if BDebugCwDecoder or BDebugGhosting then
            {$ifdef FPC}
            if Globals.GSBar <> nil then
              Globals.GSBar.Caption :=
                (Format('[%s-osFailed], Stn deleted',[MyCall]) + '; ' +
                Globals.GSBar.Caption).Substring(0, 80);
            {$else}
...
begin
  ...
  begin
    {$ifdef FPC}
    if (Globals.GEdit2Text = '') and not (SimContest in [scArrlSS]) then
      Globals.GEdit2Text := Exch1;
    if (Globals.GEdit3Text = '') and
      ((SimContest <> scNaQp) or (Exch2 <> 'DX')) then
      Globals.GEdit3Text := Exch2;
    {$else}
