...
uses
  Generics.Defaults, Generics.Collections, Contest, Station, DxStn, Log{$ifdef FPC}, GetDataPath{$endif};
...
type
  ...
  public
    ...
    class function compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TFdCallRec) : integer; static;
  ...
  TPendingCall = record
    ...
    class function comparePendingCall({$ifdef FPC}constref{$else}const{$endif} left, right : TPendingCall) : integer; static;
  end;
...
{$ifdef DISTRIBUTION_REPORT}
  {$ifdef FPC}Clipbrd{$else}Dialogs, Vcl.Clipbrd{$endif},
{$endif}
  DXCC;
...
class function TPendingCall.comparePendingCall({$ifdef FPC}constref{$else}const{$endif} left, right : TPendingCall) : integer;
begin
...
{$ifdef FPC}
function ComparePendingCallWrapper({$ifdef FPC}constref{$else}const{$endif} left, right: TPendingCall): integer;
begin
  Result := TPendingCall.comparePendingCall(left, right);
end;
{$endif}
...
begin
  ...
  PendingCallComparer := TComparer<TPendingCall>.Construct({$ifdef FPC}ComparePendingCallWrapper{$else}TPendingCall.comparePendingCall{$endif});
end;
...
var
  ...
  {$ifdef FPC}Len: integer; Str: String;{$endif}
begin
  ...
  try
    ...
    begin
      {$ifdef FPC}Len{$else}var Len : integer{$endif} := (item.Value div CallsPerDot);
      {$ifdef FPC}Str{$else}var Str : String{$endif} := StringOfChar('*', Len);
...
class function TFdCallRec.compareCall({$ifdef FPC}constref{$else}const{$endif} left, right: TFdCallRec) : integer;
begin
