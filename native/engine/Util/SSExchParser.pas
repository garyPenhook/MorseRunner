...
uses
  Lexer,
  {$ifdef FPC}Generics.Collections{$else}System.Generics.Collections{$endif},    // TObjectList
  {$ifdef FPC}Classes{$else}System.Classes{$endif},                 // TStringList
  PerlRegEx;      // for regular expression support (TPerlRegEx, TPerlRegExList)

type
  TExchTokenType = (
    ttInvalid,  // cleared/uninitialized token
...
begin
  ...
  Sections := TStringList.Create{$ifndef FPC}(False){$endif};
  ...
  Sections.Sort;
  {$ifdef FPC}Sections.Sorted := True;{$endif}  // FPC: Sort() doesn't set Sorted flag, needed for Find()
end;
...
{$ifdef FPC}var Index: Integer;{$endif}
begin
  {$ifdef FPC}Index{$else}var Index: Integer{$endif} := Reg.NamedGroup(UTF8String(name));
  assert(Index >= 0);
...
{$endif}
  ...
  try
    ...
    {$ifndef FPC}begin{$endif}
      {$ifndef FPC}var token0: TExchToken;{$endif}
    ...
    {$ifdef FPC}SkipNextToken{$else}var SkipNextToken: Boolean{$endif} := False;
    {$ifndef FPC}var I: Integer;{$endif}
